...
public class CircleImageView extends AppCompatImageView {
    ...
    public void setScalType(ScaleType scalType){
        if(scalType != SCALE_TYPE){
            throw new IllegalArgumentException(String.format("ScaleType %s not supported.", scalType));
        }
    }
    ...
    protected void onDraw(Canvas canvas){
        ...
        if (mShowFlag && mFlagText != null){
            ...
            canvas.drawText(mFlagText, getWidth() / 2, (float) ((3 + cos((float)(Math.PI * 5 / 18))) * getHeight() / 4 + mFlagTextBounds.height()/3), mFlagTextPaint);
        }
    }
    ...
    //邊框厚度
    public void setDefaultBorderWidth(int borderWidth){
